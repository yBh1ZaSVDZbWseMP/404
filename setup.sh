...
clear;clear;clear
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m          WELCOME TO SRICPT BY ELVARETTA             \033[0m"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 3
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/elvaretta-store/izinsc/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/elvaretta-store/izinsc/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/elvaretta-store/izinsc/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/yBh1ZaSVDZbWseMP/404/main/"
start=$(date +%s)
secs_to_human() {
...
echo "ELVARETTA STORE" > /etc/xray/username
echo ""
elif [[ $host == "2" ]]; then
echo "ELVARETTA STORE" > /etc/xray/username
...
USRSC=$(wget -qO- https://raw.githubusercontent.com/elvaretta-store/izinsc/main/ip | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/elvaretta-store/izinsc/main/ip | grep $ipsaya | awk '{print $3}')
TIMEZONE=$(printf '%(%H:%M:%S)T')
TEXT="
<code>────────────────────</code>
<b> 🟢 NOTIFICATIONS INSTALL 🟢</b>
<b>    🟢 SCRIPT VERSI 404 🟢  </b>
<code>────────────────────</code>
...
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://t.me/budi_spielberg"},{"text":"Contack","url":"https://wa.me/6285173028866"}]]}'
...
wget raw.githubusercontent.com/yBh1ZaSVDZbWseMP/404/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
cd
...
cd /bin
git clone  https://github.com/diah082/wondershaper.git
cd wondershaper
sudo make install
cd
rm -rf wondershaper
...
wget -O /usr/bin/ws "https://raw.githubusercontent.com/yBh1ZaSVDZbWseMP/404/main/Fls/ws" >/dev/null 2>&1
wget -O /usr/bin/tun.conf "https://raw.githubusercontent.com/yBh1ZaSVDZbWseMP/404/main/Cfg/tun.conf" >/dev/null 2>&1
wget -O /etc/systemd/system/ws.service "https://raw.githubusercontent.com/yBh1ZaSVDZbWseMP/404/main/Fls/ws.service" >/dev/null 2>&1
...
wget -q -O /usr/bin/enc "https://raw.githubusercontent.com/yBh1ZaSVDZbWseMP/404/main/Enc/encrypt" ; chmod +x /usr/bin/enc
7z x -p-menu menu1.zip
chmod +x menu1/*
enc menu1/*
...
wget https://raw.githubusercontent.com/yBh1ZaSVDZbWseMP/404/main/Cfg/m-noobz 
